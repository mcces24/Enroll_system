...
server with default setting (user 'root' with no password) */
    ...
    if (isset($_REQUEST["term"])) {
        ...
        $sql = "SELECT * FROM students INNER JOIN shift_students s ON students.id_number = s.id_number INNER JOIN course c ON students.course_id = c.course_id INNER JOIN year_lvl y ON students.year_id = y.year_id WHERE semester_id = '$semester' AND academic = '$academic' AND (status_type = 'Shift Students' OR status_type = 'Shift Clear') AND course_code = 'BEED' AND students.id_number LIKE ? LIMIT 5";
