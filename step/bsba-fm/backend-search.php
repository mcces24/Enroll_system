...
if(isset($_REQUEST["term"])){
    ...
    $sql = "SELECT * FROM students INNER JOIN que q ON students.id=q.student_id INNER JOIN admission_list a ON students.applicant_id = a.applicant_id WHERE semester_id = '$semester' AND academic = '$academic' AND status_type = 'Applicant' AND wish_course = 'BSBA' AND students.applicant_id LIKE ? LIMIT 5";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) > 0){
                ...
                while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
                    ?>
                    <p><a href="index.php?search=<?php echo $row["applicant_id"] ?>"><?php echo $row["applicant_id"] ?></a></p>
                    <?php
                }
            } else{
