...
$sql_new="SELECT students.applicant_id as app_id, * from students INNER JOIN year_lvl y On students.year_id=y.year_id INNER JOIN course c ON students.course_id=c.course_id INNER JOIN que ON students.id = que.student_id WHERE semester_id = '$semester' AND academic = '$academic' AND  status_type = 'Enroll' ORDER BY students.id DESC LIMIT 1";
echo $sql_new;
$query_new = mysqli_query($conn,$sql_new);
$new= mysqli_fetch_assoc($query_new);
