...
$sql_new="SELECT students.applicant_id as app_id from students INNER JOIN que q ON q.student_id = students.id INNER JOIN year_lvl y On students.year_id=y.year_id INNER JOIN course c ON students.course_id=c.course_id WHERE semester_id = '$semester' AND academic = '$academic' AND  status_type = 'Enroll' ORDER BY id DESC LIMIT 1";
$query_new = mysqli_query($conn,$sql_new);
$new= mysqli_fetch_assoc($query_new);
