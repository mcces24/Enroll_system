...
$query = $conn->query("
    ...
    FROM 
        students 
    INNER JOIN 
        student_acc s ON students.id_number = s.id_number 
    INNER JOIN 
        course c ON students.course_id = c.course_id 
    INNER JOIN 
        year_lvl y ON students.year_id = y.year_id 
    ORDER BY 
        students.id ASC
");
...
if ($query && $query->num_rows > 0) {
    ...
    echo '<script>
            alert("No record found with the selected academic year and semester!");
